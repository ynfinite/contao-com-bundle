...
/**
 * Table tl_ynfinite_form_fields
 */
$GLOBALS['TL_DCA']['tl_ynfinite_form_fields'] = array
(
	...
	(
		...
		(
			array('tl_ynfinite_form_fields', 'checkPermission')
		),
		'sql' => array
		(
			'keys' => array
	...
	(
		...
		(
			...
			'child_record_callback'   => array('tl_ynfinite_form_fields', 'listFormFields')
		),
		...
		(
			...
			(
				...
				'button_callback'     => array('tl_ynfinite_form_fields', 'toggleIcon')
			),
			'show' => array
			(
	...
	(
		...
		'pid' => array
		(
			'foreignKey'              => 'tl_ynfinite_form.title',
		...
		(
			...
			'filter'                  => true,
			'inputType'               => 'select',
			'options_callback'        => array('tl_ynfinite_form_fields', 'getFields'),
		...
		(
			'label'                   => &$GLOBALS['TL_LANG']['tl_ynfinite_form_fields']['contentTypeField'],
			'exclude'                 => true,
			'filter'                  => true,
			'inputType'               => 'select',
			'options_callback'        => array('tl_ynfinite_form_fields', 'getContentTypeFields'),
			'eval'                    => array('mandatory' => true, 'tl_class'=>'w100 clr', 'submitOnChange'=> true),
			'sql'                     => "varchar(64) NOT NULL default ''"
		),
		'operation' => array
		(
			'label'                   => &$GLOBALS['TL_LANG']['tl_ynfinite_form_fields']['operation'],
			'default'                 => 'text',
			'exclude'                 => true,
			'filter'                  => true,
			'inputType'               => 'select',
			'options_callback'        => array('tl_ynfinite_form_fields', 'getOperations'),
		...
		(
            ...
            'options_callback'     => array('tl_ynfinite_form_fields', 'getOptions'), 
            'exculde'           	=> true,
		...
		(
			...
			'exclude'                 => true,
			'inputType'               => 'select',
			'options_callback'        => array('tl_ynfinite_form_fields', 'getFormFieldTemplates'),
 ...
 */
class tl_ynfinite_form_fields extends Backend
{
	...
	public function checkPermission()
	{
		...
		switch (Input::get('act'))
		{
			...
			case 'copy':
				...
				if (Input::get('mode') == 1)
				{
					$objField = $this->Database->prepare("SELECT pid FROM tl_ynfinite_form_fields WHERE id=?")
											   ->limit(1)
			...
			case 'copyAll':
				...
				$objForm = $this->Database->prepare("SELECT id FROM tl_ynfinite_form_fields WHERE pid=?")
										  ->execute($id);
...
<div class="limit_height' . (!Config::get('doNotCollapse') ? ' h32' : '') . '">';
	...
	{
		$arrFields = $GLOBALS['TL_YNFINITE_FORM_TYPES'];
		// Add the translation
	...
	public function toggleVisibility($intId, $blnVisible, DataContainer $dc=null)
	{
		...
		if (!$this->User->hasAccess('tl_ynfinite_form_fields::invisible', 'alexf'))
		{
		...
		if ($dc)
		{
			$objRow = $this->Database->prepare("SELECT * FROM tl_ynfinite_form_fields WHERE id=?")
									 ->limit(1)
									 ->execute($intId);
		...
		$objVersions = new Versions('tl_ynfinite_form_fields', $intId);
		...
		// Update the database
		$this->Database->prepare("UPDATE tl_ynfinite_form_fields SET tstamp=$time, invisible='" . ($blnVisible ? '1' : '') . "' WHERE id=?")
					   ->execute($intId);
	...
	public function getContentTypeFields(DataContainer $dc)
	{
		if($dc->activeRecord->pid) {
			$objFilter = $this->Database->prepare("SELECT * FROM tl_ynfinite_form WHERE id=?")
											   ->limit(1)
	...
	public function getOperations() {
		$arrOperations = $GLOBALS['TL_YNFINITE_OPERATIONS'];
