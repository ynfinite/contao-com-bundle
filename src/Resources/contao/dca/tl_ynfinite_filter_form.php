...
/**
 * Table tl_ynfinite_filter_form
 */
$GLOBALS['TL_DCA']['tl_ynfinite_filter_form'] = array
(
	...
	(
		...
		'ctable'                      => array('tl_ynfinite_filter_form_fields'),
		'onload_callback' => array
		(
			array('tl_ynfinite_filter_form', 'checkPermission')
		),
		'sql' => array
		(
			'keys' => array
	...
	(
		...
		(
			...
			(
				...
				'href'                => 'table=tl_ynfinite_filter_form_fields',
				'icon'                => 'edit.svg'
			),
			'editheader' => array
			(
				...
				'href'                => 'act=edit',
				'icon'                => 'header.svg',
				'button_callback'     => array('tl_ynfinite_filter_form', 'editHeader')
			),
			'copy' => array
			(
				...
				'href'                => 'act=copy',
				'icon'                => 'copy.svg',
				'button_callback'     => array('tl_ynfinite_filter_form', 'copyForm')
			),
			'delete' => array
			(
				...
				'button_callback'     => array('tl_ynfinite_filter_form', 'deleteForm')
			),
			'show' => array
			(
	...
	(
		...
		(
			...
			'save_callback' => array
			(
				array('tl_ynfinite_filter_form', 'generateAlias')
			),
		...
		'contentType'             => array(
            ...
            'inputType'         => 'select',
            'options_callback'  => array('tl_ynfinite_filter_form', 'getContentTypes'),
            'exculde'           => true,
            'eval'              => array(
                'maxLength'     => 255,
		...
		(
			...
			'search'                  => true,
			'inputType'               => 'select',
			'options_callback'        => array('tl_ynfinite_filter_form', 'getAllTables'),
		...
		(
			...
			'exclude'                 => true,
			'inputType'               => 'select',
			'options_callback'        => array('tl_ynfinite_filter_form', 'getFormWrapperTemplates'),
 ...
 */
class tl_ynfinite_filter_form extends Backend
{
	...
	/**
	 * Check permissions to edit table tl_ynfinite_filter_form
	 *
		...
		switch (Input::get('act'))
		{
			...
			case 'edit':
				...
				if (!in_array(Input::get('id'), $root))
				{
					...
					if (is_array($arrNew['tl_ynfinite_filter_form']) && in_array(Input::get('id'), $arrNew['tl_ynfinite_filter_form']))
					{
	...
	public function generateAlias($varValue, DataContainer $dc)
	{
		...
		$objAlias = $this->Database->prepare("SELECT id FROM tl_ynfinite_filter_form WHERE id=? OR alias=?")
	...
	{
		return $this->User->canEditFieldsOf('tl_ynfinite_filter_form') ? '<a href="'.$this->addToUrl($href.'&amp;id='.$row['id']).'" title="'.StringUtil::specialchars($title).'"'.$attributes.'>'.Image::getHtml($icon, $label).'</a> ' : Image::getHtml(preg_replace('/\.svg$/i', '_.svg', $icon)).' ';
	}
